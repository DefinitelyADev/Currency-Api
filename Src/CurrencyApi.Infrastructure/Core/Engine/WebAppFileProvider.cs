...
namespace CurrencyApi.Infrastructure.Core.Engine
{
    ...
    /// </summary>
    public class WebAppFileProvider : PhysicalFileProvider, IWebAppFileProvider
    {
        /// <summary>
        /// Initializes a new instance of a WebAppFileProvider
        /// </summary>
        /// <param name="hostEnvironment">Hosting environment</param>
        public WebAppFileProvider(IHostEnvironment hostEnvironment) : base(File.Exists(hostEnvironment.ContentRootPath) ? Path.GetDirectoryName(hostEnvironment.ContentRootPath) : hostEnvironment.ContentRootPath)
        {
            WebRootPath = File.Exists(hostEnvironment.ContentRootPath) ? Path.GetDirectoryName(hostEnvironment.ContentRootPath) : hostEnvironment.ContentRootPath;
        }
        ...
        /// <returns>
        /// <see langword="true" /> if the path is a valid UNC path;
        ...
        public virtual string GetAbsolutePath(params string[] paths)
        {
            ...
            if (WebRootPath != null && paths.Any() && !paths[0].Contains(WebRootPath, StringComparison.InvariantCulture))
            {
